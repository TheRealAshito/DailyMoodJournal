    ...
    <div className="card-bg border border-custom rounded-xl p-4">
      ...
      {entry.tags && entry.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2">
          {entry.tags.map((tag) => (
            <span key={tag} className="px-2 py-0.5 rounded-full text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">{tag}</span>
          ))}
        </div>
      )}
      ...
      <div className="flex gap-2 mt-3 pt-3 border-t border-custom">
        <button onClick={() => navigate(`/edit/${encodeURIComponent(entry.path)}`)} className="text-sm text-purple-600 hover:underline">{t('edit')}</button>
