...
from utils import build_entry_path, parse_entry_text, build_entry_text, list_user_entries
...
def get_entries_for_date(username: str, date_obj) -> list[dict]:
    entries = []
...
def get_all_tags_for_user(username: str) -> list[str]:
    tags_set = set()
    for path in list_user_entries(username):
